...
`Baseline`은 비교할 이전 벤치마크 결과 파일 경로이며, 없으면 `none`입니다. `Performance-Critical Functions`는 성능 목표를 지켜야 하는 함수 목록입니다.

## Procedure

### Step 1 — Identify Performance-Critical Functions

입력에 명시된 함수, SPEC이 지정한 경로, 그리고 프로젝트가 @AX 태그를 사용하는 경우 `@AX:ANCHOR` 지점을 후보로 모읍니다.

### Step 2 — Benchmark, Profile, Compare

Stack Profile이 주입되면 그 명령을 사용하고, 없으면 스택 표준 도구를 씁니다.

| Stack | Benchmark | Profile | Baseline 비교 |
|-------|-----------|---------|---------------|
| Go | `go test -bench=. -benchmem -count=5 ./pkg/target/...` | `-cpuprofile` + `go tool pprof` | `benchstat` |
| Python | `pytest --benchmark-only` | `py-spy` 또는 `cProfile` | `pytest --benchmark-compare` |
| Node.js | `npx vitest bench` | `npx clinic doctor` | JSON diff |
| Rust | `cargo bench` | `cargo flamegraph` | `critcmp` |

결과는 비교 가능한 파일로 저장합니다: `<benchmark-command> | tee bench_$(date +%Y%m%d_%H%M%S).txt`

분석 초점: CPU 5% 이상 소비 함수, 과도한 allocs/op, 동시성 경합.

### Step 3 — Interpret

- **Regression**: 10% 이상 감속 또는 20% 이상 메모리 증가
- **Improvement**: 10% 이상 가속
- **Neutral**: ±10% 이내 변동
- baseline이 없으면 현재 결과를 baseline으로 저장하고 출력에 명시합니다.

### Step 4 — Report Regressions and Suggestions

회귀마다 함수/벤치마크 이름, before/after 지표(ns/op, B/op, allocs/op), 프로파일 근거에 기반한 원인 가설, 구체적 최적화 제안을 적습니다. 제안은 alloc 감소, hot path의 불필요한 wrapping 제거, 안전한 경우 lock → atomic 전환, 버퍼드 I/O, 직렬화 비용 축소처럼 실행 가능한 형태로 씁니다.
