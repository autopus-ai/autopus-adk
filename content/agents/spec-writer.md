...
SPEC Markdown files under `.autopus/specs/**` are documentation: 줄 수만으로 분할하거나 거절하지 않습니다. 프로젝트 줄 수 한도는 SPEC이 참조하는 구현 소스 코드 파일에만 적용됩니다.
...
1. `.claude/rules/autopus/spec-quality.md`의 체크리스트를 읽고 `spec.md`, `plan.md`, `acceptance.md`, `research.md`에 적용합니다.
2. 판정은 `research.md`의 `## Self-Verify Summary`에 `Q-* | status | attempt | files | reason` 형식으로 남기고, 재검증하면 최신 상태로 갱신합니다.
3. FAIL은 증상이 보인 파일만 고치지 말고 원인 차원이 요구하는 모든 파일을 수정합니다. `[NEW]` 마커가 붙은 planned addition은 코드 정합성 FAIL 대상이 아니며, 기존 참조만 실제 경로·이름을 검증합니다.
4. 체크리스트는 최대 2회까지 다시 적용하고, 남은 FAIL은 `spec.md` 말미 `## Open Issues`에 `Q-* | category | scope | attempt | reason` 형식으로 기록합니다.
5. 4개 파일 저장 후 `auto spec validate {SPEC_DIR} --strict`를 실행합니다(로컬 source tree에서는 `go run ./cmd/auto spec validate {SPEC_DIR} --strict`도 가능). deterministic finding이 남아 있으면 provider review로 넘기지 않고 먼저 수정합니다.
...
- 요구사항은 EARS 형식, 수락 기준은 bare Given/When/Then 형식
- research.md는 실제 코드 경로와 함수명을 인용하고 existing reference와 `[NEW]` planned addition을 구분
- Must oracle acceptance는 concrete expected output 또는 explicit tolerance를 포함
- plan.md의 태스크는 독립적으로 실행 가능한 단위이며 Outcome Lock을 닫는 작업을 포함
- greenfield 요청이면 `## Technology Stack Decision`에 concrete stable version, source ref, checked_at을 기록
- 필수 섹션 목록은 위 `### 4. SPEC 파일 생성` 템플릿과 `auto spec validate --strict`가 정의하며 여기서 다시 열거하지 않습니다
