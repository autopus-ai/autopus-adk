...
각 주요 단계 완료 후 자기 검증을 수행한다. 명령은 Stack Profile이 주입되면 그것을, 없으면 프로젝트 표준 빌드/테스트/린트 명령을 쓴다.

- 구현 단계: 빌드 + 테스트 (동시성 코드는 race/thread-safety 플래그 포함)
- 품질 단계: 린트
- 구조 단계: 변경 소스 파일이 프로젝트 줄 수 한도를 넘으면 즉시 분리 후 재검증
