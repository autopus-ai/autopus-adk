...
Stack Profile이 주입되면 그 명령을 사용하고, 없으면 project context(`.autopus/project/tech.md`, `go.mod`, `package.json`, `pyproject.toml`, `Cargo.toml`)에서 스택을 감지해 프로젝트 표준 명령을 찾는다.

1. 빌드
2. 테스트 (동시성 코드는 race/thread-safety 플래그 포함)
3. 린트
4. 커버리지 — 기본 임계값 85%, 프로젝트 선언값 우선 (`0`이면 수치 게이트 해제)

### 5. 구조 검증
- 프로젝트가 명시한 소스 파일 줄 수 한도(`file-size-limit` 규칙) 초과 파일
- 한도에 근접한 파일 목록
...
CLI/API 프로젝트는 빌드 산출물을 실제로 실행해 확인합니다. entry point는 `.autopus/project/scenarios.md`의 Binary/Build 필드 또는 프로젝트 manifest의 main에서 추출합니다.

- CLI: `--help` 실행 — exit 0, stdout non-empty
- API: 서버 기동 후 health 엔드포인트 호출 — HTTP 200
...
**Detection method**: 변경 파일에서 클라이언트 호출(`fetch`, `axios`, `http.Get/Post`, `requests.post` 등)과 서버 라우트 등록(`app.post`, `HandleFunc`, `@app.post` 등)을 각각 추출합니다.
...
- Markdown 섹션 구조 검증
- 줄 수 한도는 소스 코드 파일에만 적용하므로 SPEC/agent Markdown 문서에는 적용하지 않음
...
| 커버리지 게이트 미달 | tester | 미커버 경로에 테스트 추가 |
...
Check 6a Layer 2(behavioral stub)와 Check 7(acceptance coverage)은 함수 body와 SPEC 시나리오를 읽고 판단해야 하므로, 파이프라인 orchestrator가 프로젝트 quality preset의 상위 추론 프로파일로 이 체크를 실행합니다. 실제 모델/플랫폼 매핑은 quality preset 설정과 플랫폼 어댑터가 소유하며 이 프롬프트에 복제하지 않습니다.

- Checks 1-5, 6a Layer 1, 6b, 6c → 기본 프로파일
- Check 6a Layer 2, Check 7 → 상위 추론 프로파일

## Multi-Model Validation (--multi)

`--multi`이면 Gate 2가 서로 다른 추론 프로파일로 validator 둘을 병렬 스폰하고, verdict는 strict-union(어느 쪽이든 FAIL이면 FAIL)으로 합칩니다. 각 validator는 모든 체크를 독립 수행하고, 이슈 목록은 함수명/acceptance ID로 중복 제거합니다.
