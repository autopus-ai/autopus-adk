...
@AX annotation is not a preset-driven phase. It is an explicit opt-in gate whose
default is `annotation: not_applicable — @AX annotation not requested for this change set`,
so `annotator` runs only when the gate is requested, regardless of preset membership.
